...
\header {
  ...
  oll-description = \markup \justify {
    When preparing diplomatic editions of sketches and drafts one usually
    wants to indicate the line breaks of the original source. This can be
    achieved by either duplicating the original breaks in the engraving or
    by indicating the positions of the breaks with dedicated graphical signs.

    This snippet draws a dashed vertical line above the staff, which is
    a common notation for this purpose. Its use is independent from barlines
    and it can appear at any position in the measure. Internally a custom
    rehearsal mark is used.
  }
